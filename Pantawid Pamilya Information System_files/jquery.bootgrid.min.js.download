! function(a, b, c) {
    ...
    if (I.defaults = {
            ...
            css: {
                ...
                infos: "infos",
                left: "text-left",
                pagination: "pagination pg-blue mb-0",
                paginationButton: "page-link",
                responsiveTable: "table-responsive",
                right: "text-right",
                search: "search form-group",
                searchField: "search-field form-control",
                selectBox: "form-check-input",
                selectCell: "select-cell",
                selected: "active",
                sortable: "sortable",
                table: "bootgrid-table table table-hover"
            },
            formatters: {},
            labels: {
                all: "All",
            ...
            templates: {
                ...
                paginationItem: '<li class="page-item"><a data-page="{{ctx.page}}" class="{{css.paginationButton}}">{{ctx.text}}</a></li>',
                rawHeaderCell: '<th class="{{ctx.css}}">{{ctx.content}}</th>',
                row: "<tr{{ctx.attr}}>{{ctx.cells}}</tr>",
                search: '<div class="{{css.search}}"><div class="input-group"><span class="{{css.icon}} input-group-addon {{css.iconSearch}}"></span> <input type="text" class="{{css.searchField}}" placeholder="{{lbl.search}}" /></div></div>',
                select: '<div class="form-check"><input name="select[]" type="{{ctx.type}}" class="{{css.selectBox}}" value="{{ctx.value}}" {{ctx.checked}} id="materialUnchecked{{ctx.value}}" /><label class="form-check-label" for="materialUnchecked{{ctx.value}}"></label></div>'
            }
